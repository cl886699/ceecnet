...
class FrozenBatchNormalization(tf.keras.layers.Layer):
    def __init__(self,
                 axis=-1,
                 epsilon=1e-3,
                 momentum=0.9,
    ...
    def call(self, inputs, training=None):
        gamma, beta = self._get_reshaped_weights(tf.keras.backend.int_shape(inputs))
        mean = tf.reshape(self.moving_mean, tf.shape(gamma))
        variance = tf.reshape(self.moving_variance, tf.shape(gamma))

        # mean, variance = tf.nn.moments(
        #     tf.cast(inputs, self._param_dtype), axes=self.axes, keepdims=True)
        ...
        return inputs * tf.cast(inv, inputs.dtype) + tf.cast(
            beta - mean * inv if self.center else -1. * mean * inv, inputs.dtype)
    ...
    def get_config(self):
        ...
        config.update(super(FrozenBatchNormalization, self).get_config())
...
class FilterResponseNormalization(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if len(tf.TensorShape(input_shape)) != 4:
            raise ValueError(
                """Only 4-D tensors (CNNs) are supported as of now.""")
        self._check_if_input_shape_is_none(input_shape)
        self._create_input_spec(input_shape)
        self._add_gamma_weight(input_shape)
        self._add_beta_weight(input_shape)
    ...
    def _check_axis(self, axis):
        if not isinstance(axis, list):
            raise TypeError("""Expected a list of values but got {}.""".format(type(axis)))
        else:
            self.axis = axis
